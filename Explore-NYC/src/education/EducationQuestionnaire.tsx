...
const LOOKING_FOR_OPTIONS = [
  { value: 'event', label: 'Professional Events', sub: 'Workshops, bootcamps, seminars & programs' },
  { value: 'job',   label: 'Jobs & Internships',  sub: 'Job boards, fellowships & career placement' },
  { value: 'both',  label: 'Both',                sub: 'Show me everything' },
];
...
export default function EducationQuestionnaire() {
  ...
  const [prefs, setPrefs] = useState<EducationPreferences>({
    lookingFor: '' as EducationPreferences['lookingFor'],
    level: '',
    focusArea: '',
    experienceYears: -1,
    extraSearch: '',
  ...
  const canProceed = (() => {
    ...
    if (step === 1) return prefs.level !== '';
    if (step === 2) return prefs.focusArea !== '';
    if (step === 3) return prefs.experienceYears >= 0;
    return true;
  })();
        ...
        {step === 0 && (
          <div className="flex flex-col flex-1">
            ...
            <p className="text-sm" style={{ color: 'rgba(255,255,255,0.45)' }}>
              Choose what you want to discover in NYC
            </p>
            <div className="flex-1 flex items-center justify-center">
              <div className="flex flex-col gap-3 w-full max-w-sm">
                {LOOKING_FOR_OPTIONS.map(opt => (
                  <button
                    key={opt.value}
                    onClick={() => setPrefs(p => ({ ...p, lookingFor: opt.value as EducationPreferences['lookingFor'] }))}
                    className="w-full rounded-2xl text-left transition-all duration-150 hover:opacity-90 active:scale-95"
                    style={{
                      padding: '14px 20px',
                      backgroundColor: prefs.lookingFor === opt.value ? '#4A9EE0' : 'rgba(255,255,255,0.12)',
                      border: prefs.lookingFor === opt.value ? '2px solid #4A9EE0' : '2px solid transparent',
                    }}
                  >
                    <p className="text-base font-semibold" style={{ color: '#fff' }}>{opt.label}</p>
                    <p className="text-xs mt-0.5" style={{ color: prefs.lookingFor === opt.value ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.45)' }}>
                      {opt.sub}
                    </p>
                  </button>
                ))}
        ...
        {step === 3 && (
          <div className="flex flex-col flex-1 items-center">
            <h2 className="text-2xl font-bold tracking-wide w-full" style={{ color: 'rgba(255,255,255,0.95)' }}>
              {prefs.lookingFor === 'event' ? 'Your experience level' : 'Job experience'}
            </h2>
            <p className="text-sm w-full mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>
              {prefs.lookingFor === 'event'
                ? 'Helps match you to the right programs'
        ...
        {step === 4 && (
          <div className="flex flex-col flex-1">
            ...
            <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>
              {prefs.lookingFor === 'event'
                ? 'Specific skill, topic, or program? (optional)'
                : 'Specific role, company type, or skill? (optional)'}
            </p>
            <div style={{ height: '24px' }} />
            <div className="relative">
              <input
                ...
                placeholder={prefs.lookingFor === 'event'
                  ? 'e.g. bootcamp, AI, robotics…'
                  : 'e.g. internship, data analyst, startup…'}
