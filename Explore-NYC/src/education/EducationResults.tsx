...
export default function EducationResults() {
  ...
  const [filterLevel,   setFilterLevel]   = useState(preferences?.level ?? '');  // display chip only
    ...
    <div className="min-h-screen flex flex-col items-center" style={{ padding: '32px 24px' }}>
      <div className="w-full max-w-3xl flex flex-col">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => exportEducationToPDF(results)}
        ...
        {showFilters && (
          <div className="rounded-3xl mt-3" style={{ backgroundColor: '#1a3a5c', padding: '24px' }}>
            ...
            {/* Status */}
            <p className="text-xs uppercase tracking-widest font-semibold mb-3"
              style={{ color: 'rgba(255,255,255,0.5)' }}>REGISTRATION STATUS</p>
        ...
        {results.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {results.map(org => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-semibold rounded-full self-start"
                      ...
                      <>
                        ...
                        {org.dueDate && (
                          <span className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>
                            · Deadline {formatDueDate(org.dueDate)}
                          </span>
                        )}
                      </>
                    ) : (
          ...
          <div className="rounded-3xl text-center" style={{ backgroundColor: '#1a3a5c', padding: '40px 24px' }}>
            <p className="text-lg font-semibold" style={{ color: 'rgba(255,255,255,0.85)' }}>No organizations found</p>
            <div style={{ height: '10px' }} />
        ...
        >
          ...
          >
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 pr-3">
                  ...
                  <h2 className="font-bold leading-snug"
                    style={{ fontSize: '18px', color: '#fff', fontFamily: 'Playfair Display, Georgia, serif' }}>
                    {selectedOrg.name}
                  </h2>
                </div>
            ...
            <div style={{ padding: '20px' }} className="flex flex-col gap-3">
              <div>
                <p className="text-xs uppercase tracking-widest font-semibold mb-1" style={{ color: '#999' }}>Program</p>
              ...
              <div className="flex items-center gap-2">
                {isOpen(selectedOrg.dueDate) ? (
                  <>
                    ...
                    <span className="text-sm font-semibold" style={{ color: '#16a34a' }}>Registration Open</span>
                    {selectedOrg.dueDate && (
                      <span className="text-xs" style={{ color: '#999' }}>· Deadline {formatDueDate(selectedOrg.dueDate)}</span>
                    )}
                  </>
                ) : (
                  <>
                    <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: '#f87171' }} />
                    <span className="text-sm font-semibold" style={{ color: '#dc2626' }}>Registration Closed</span>
                    {selectedOrg.dueDate && (
              ...
              {selectedOrg.registrationLink && (
                ...
                >
                  {isOpen(selectedOrg.dueDate) ? 'Register Now ↗' : 'Registration Closed'}
                </a>
              )}
